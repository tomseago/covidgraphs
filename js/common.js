...
// 2020 Pop and Growth since 2018
...
function addSortBy(by, dataField) {
    // Never sort by strings - the only one being the state name
    stateData.sort((a,b) => {
        // b - a so it is descending
        return parseFloat(b[dataField]) - parseFloat(a[dataField]);
    });

    // Now list the names
    sortedStateNames[by] = stateData.map(sd => sd.State);
}

sortedStateNames.alpha = stateNames;
addSortBy("pop", "Pop");
addSortBy("density", "density");
addSortBy("percent", "Percent");
